...
public class MenuRestController {
	...
	@GetMapping("/type/{type}")  // {type}  -->  ${e.target.value}
...
//		// 사용자가 id를 db에 있는 id를 넣었을 때는 정상조회
//		// 사용자가 id를 db에 없는 id를 넣었을 때는 500오류(서버측 오류)
//	}
	/*
	...
	public ResponseEntity<Menu> findById(@PathVariable("id") Long id) {
		...
		if(menu.isPresent()) {
			...
			return ResponseEntity.ok(menu.get());  // 위코드와 동일한데 이렇게 더 많이 씀
		}else {
